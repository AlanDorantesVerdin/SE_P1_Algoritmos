...
# ─── PALETA DE COLORES (Tema: Roadtrip Pueblos Mágicos) ─────────────────
BG        = '#1E1E24'   # Asfalto oscuro
PANEL_BG  = '#2B2D42'   # Panel nocturno
BORDER    = '#8D99AE'   # Gris acero (accento)
GREEN_MST = '#00FA9A'   # Verde esmeralda (Ruta segura)
YELLOW    = '#FFD166'   # Amarillo (Rutas evaluadas)
ORANGE    = '#F4A261'   # Naranja (Evaluación actual)
RED       = '#E63946'   # Rojo (Bucle descartado, ruta insegura/redundante)
GRAY_E    = '#4A4E69'   # Gris ruta inactiva
WHITE     = '#EDF2F4'   # Texto principal
DIM       = '#8D99AE'   # Texto secundario
ISLANDS   = ['#A8DADC', '#457B9D', '#1D3557', '#E0FBFC', '#98C1D9', '#3D5A80']

class KruskalRoadtrip:
    ...
    def kruskal_with_steps(self):
        ...
        # Cambio clave: reverse=True para buscar el Árbol de Expansión MÁXIMA
        sorted_edges = sorted([(d['weight'], u, v) for u, v, d in self.graph.edges(data=True)], reverse=True)
        mst_edges, total_weight = [], 0

        self.steps.append({'current_edge': None, 'mst_edges': [], 'total_weight': 0, 'parent': dict(parent),
                           'message': 'Mapeo inicial. Rutas clasificadas por nivel de seguridad (Mayor es mejor).'})

        for w, u, v in sorted_edges:
            self.steps.append({'current_edge': (u, v, w), 'mst_edges': mst_edges.copy(),
                'total_weight': total_weight, 'parent': dict(parent), 'message': f'Fase de evaluación: {u} ↔ {v} (Seguridad: {w})'})
            ...
            if union(u, v):
                ...
                self.steps.append({'current_edge': (u, v, w), 'mst_edges': mst_edges.copy(),
                    ...
                    'message': f'Ruta {u} ─ {v} agregada al itinerario seguro. (+{w} pts)'})
            else:
                self.steps.append({'current_edge': (u, v, w), 'mst_edges': mst_edges.copy(),
                    'total_weight': total_weight, 'parent': dict(parent), 'rejected': True,
                    'message': f'Ruta {u} ─ {v} descartada (Destinos ya conectados por rutas más seguras).'})

        self.steps.append({'current_edge': None, 'mst_edges': mst_edges.copy(),
            'total_weight': total_weight, 'parent': dict(parent), 'final_mst': True,
            'message': 'Itinerario Roadtrip definido. Ruta más segura establecida.'})
    ...
    def visualize(self):
        ...
        self.fig.suptitle('ROADTRIP PUEBLOS MÁGICOS (KRUSKAL ÁRBOL MÁXIMO)', fontsize=18, fontweight='bold', color=WHITE, x=0.5, y=0.97, ha='center')
        self.pos = nx.spring_layout(self.graph, seed=84, k=4, iterations=150)
    ...
    def draw_step(self):
        ...
        # ── SEPARACIÓN DE ARISTAS (GLOW EFECTO VIAJE) ──
        normal_edges = []
        glow_green_edges = []
        glow_green_thick = []
        glow_orange_edges = []
        ...
        for edge in self.graph.edges():
            ...
            if in_mst and not is_cur: glow_green_edges.append(edge)
            elif is_cur and step.get('accepted'): glow_green_thick.append(edge)
        ...
        # 1. Halo Glow Fluido
        nx.draw_networkx_edges(self.graph, self.pos, edgelist=glow_green_edges, ax=self.ax_g, width=14, edge_color=GREEN_MST, alpha=0.15)
        nx.draw_networkx_edges(self.graph, self.pos, edgelist=glow_green_thick, ax=self.ax_g, width=18, edge_color=GREEN_MST, alpha=0.25)
        ...
        # 2. Línea Sólida Primaria (Carretera)
        nx.draw_networkx_edges(self.graph, self.pos, edgelist=glow_green_edges, ax=self.ax_g, width=4.5, edge_color=GREEN_MST, alpha=1.0)
        nx.draw_networkx_edges(self.graph, self.pos, edgelist=glow_green_thick, ax=self.ax_g, width=6.0, edge_color=GREEN_MST, alpha=1.0)
        ...
        for node in self.graph.nodes():
            ...
            abbrev = str(node)[:3].upper()
            self.ax_g.text(x, y, abbrev, color=BG if checked else WHITE, fontsize=6, fontweight='bold', ha='center', va='center', zorder=3)
            
            # Nombre completo
            self.ax_g.text(x, y - 0.03, node, ha='center', va='top', fontsize=10, color=WHITE,
                           bbox=dict(boxstyle='round,pad=0.25', facecolor=PANEL_BG, edgecolor=c, linewidth=1, alpha=0.9), zorder=4)

        # ── ETIQUETAS DE NIVEL DE SEGURIDAD ──
        edge_labels = {(u, v): f" {d['weight']} pts " for u, v, d in self.graph.edges(data=True)}
    ...
    def _draw_panel(self, step):
        ...
        # --- OBJETIVO ---
        draw_head('ITINERARIO ROADTRIP')
        draw_text("Protocolo: Kruskal (Árbol Expansión Máxima)\nMisión: Visitar todos los Pueblos Mágicos priorizando las carreteras de mayor nivel de seguridad.", DIM, size=9)
        y -= 0.03
        
        # --- ESTADO DE SIMULACIÓN ---
        draw_head('BITÁCORA DE VIAJE')
        ...
        draw_head('MÉTRICAS DE SEGURIDAD')
        if step.get('final_mst'): draw_badge(f"SEGURIDAD TOTAL: {step['total_weight']} PTS", GREEN_MST, BG)
        else: draw_badge(f"SEGURIDAD PARCIAL: {step['total_weight']} PTS", YELLOW, BG)
        
        tramos = len(step['mst_edges'])
        y -= 0.01
        draw_text(f"► Rutas añadidas: {tramos}", WHITE)
        y -= 0.03

        # --- LEYENDA VISUAL ---
        draw_head('MAPA DE CARRETERAS')
        items = [
            ('━', GREEN_MST, 'Ruta segura en el itinerario'),
            ('━', ORANGE,    'Evaluando condiciones de vía'),
            ('━', RED,       'Ruta de baja seguridad descartada'),
            ('●', GREEN_MST, 'Pueblo Mágico conectado'),
            ('●', ORANGE,    'Destino bajo revisión'),
        ]
        ...
        # Controles
        ax.text(0.5, 0.04, "[→] Siguiente Evaluación    [Q] Cerrar Mapa", color=DIM, fontsize=9, ha='center', va='center', fontweight='bold')
...
if __name__ == '__main__':
    G = nx.Graph()
    # Grafo de un roadtrip entre Pueblos Mágicos de México
    # Nodos: Pueblos, Pesos: Seguridad de la via (1-100, mayor es más seguro)
    edges = [
        ('Tequila', 'Sayulita', 88),
        ('Tequila', 'Pátzcuaro', 75),
        ('Sayulita', 'Pátzcuaro', 62),
        ('Sayulita', 'Taxco', 55),
        ('Pátzcuaro', 'Taxco', 92),
        ('Pátzcuaro', 'Valle de Bravo', 85),
        ('Taxco', 'Valle de Bravo', 96),
        ('Taxco', 'Tepoztlán', 90),
        ('Valle de Bravo', 'Tepoztlán', 78),
        ('Tepoztlán', 'Cholula', 94),
        ('Valle de Bravo', 'Cholula', 68)
    ]
    for u, v, w in edges: G.add_edge(u, v, weight=w)
    print("Controles:\n  -> (flecha derecha): Avanzar al siguiente paso\n  q: Cerrar visualización")
    KruskalRoadtrip(G).visualize()
